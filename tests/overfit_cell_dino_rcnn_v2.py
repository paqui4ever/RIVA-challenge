...
from utils.anchors import LearnableAnchorGenerator
...
def main():
    parser = argparse.ArgumentParser(description="Overfit CELL-DINO RCNN v2 on a few images")
    ...
    if args.weighted_sampling and args.random_sample: # weighted sampling only makes sense when we sample
        # Create sampler for the subset - wait, weighted sampling usually for DataLoader to pick FROM dataset
        # Here we already picked a subset. If we want to overfit, we usually just iterate the subset.
        # If we used weighted sampling to PICK the subset, that would be different.
        # But 'WeightedRandomSampler' is for the DataLoader.
        
        # Taking weights for the selected indices
    ...
    vis_dir = Path("overfit_visualizations_cell_dino")
